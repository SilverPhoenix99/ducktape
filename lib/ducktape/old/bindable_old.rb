  ...
  module Bindable
    ...
    module ClassMethods
      def bindable(name, **options)
        name = name.to_s
        metadata = find_metadata(name)
        bindings_metadata[name] = metadata = BindableAttributeMetadata.new(metadata || name, **options)
    ...
    private #--------------------------------------------------------------
      ...
      def metadata(name)
        is_a?(Class) ? singleton_class.find_metadata(name) : self.class.find_metadata(name)
      end
